...
function update_storage(storage_object, key, value) {
  browser.storage.local.get(storage_object, function (data) {
    if (data[storage_object] != null)
      data[storage_object][key] = value;
    else
      data[storage_object] = { [key]: value };
    
    browser.storage.local.set({ [storage_object]: data[storage_object] });
  });
}
...
function set_storage(storage_object, value) {
  browser.storage.local.set({ [storage_object]: value });
}
...
document_bg_custom_save.addEventListener("click", function (e) {
  if (document_bg_custom_input.value)
    browser.storage.local.set({ custom_bg: document_bg_custom_input.value });
});
...
show_border.addEventListener("click", function (e) {
  browser.storage.local.set({ show_border: e.target.checked });
});

donate.addEventListener("click", function (e) {
  browser.tabs.create({ url: "https://www.buymeacoffee.com/waymondrang" });
});
...
function handleTempColorChange(color, saveToStorage = true) {
  ...
  if (saveToStorage)
    browser.storage.local.set({ temp_accent_color: color });
...
spectrum_reset.addEventListener("click", function (e) {
  browser.storage.local.get("accent_color", function (data) {
    ...
    browser.storage.local.remove("temp_accent_color");
  });
});
...
spectrum_save.addEventListener("click", function (e) {
  if (spectrum_input.value) {
    ...
    // SAVE HUE TO STORAGE
    browser.storage.local.set({ accent_color: { hue: spectrum_input.value } });
...
try {
  browser.storage.local.get(
    ...
    function (data) {
      ...
      if (data.mode == null) {
        browser.storage.local.set({ mode: mode_dark });
      ...
      if (option == null) {
        ...
        browser.storage.local.set({ doc_bg: option });
      }
      ...
      if (invert_data == null) {
        ...
        browser.storage.local.set({ invert: invert_data });
      }
      ...
      if (button_raised == null) {
        button_raised = false;
        browser.storage.local.set({ raise_button: button_raised });
      }

      if (border_shown == null) {
        border_shown = true;
        browser.storage.local.set({ show_border: border_shown });
      }
      ...
      if (button_options == null) {
        ...
        browser.storage.local.set({ button_options: button_options });
      }
      ...
      if (data.dark_mode == null) {
        ...
        browser.storage.local.set({ dark_mode: data.dark_mode });
      }
